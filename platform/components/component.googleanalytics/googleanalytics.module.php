<?php
	...
	*/
	...
	class mgGoogleAnalytics {
		...
		# ---------------------------------------------------------------------------------------------------------------
		# (constructor)
		public function __construct($uid = false, $domain = BASEPATH, $ip = GetRemoteAddress(), $ua = false) {
			// init tracker
			$this->ga = new GoogleAnalytics\Tracker($uid, $domain);
			// init visitor
			$this->visitor = new GoogleAnalytics\Visitor();
			$this->visitor->setIpAddress($ip)
		...
		# ---------------------------------------------------------------------------------------------------------------
		# (Event)
		public function Event($category, $action, $label, $value, interaction = false) {
