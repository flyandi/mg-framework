<?php
	...
	*/
	...
	// (id's)
	define("FEED_ID_PATH", "feeds");
	
	// (properties) 
	define("FEED_CLASS", "mgFeed");
	define("FEED_PATH", "widgets/");
	define("FEED_XML", "widget.xml");
	define("FEED_SORTFILE", "feed.sort");
	define("FEED_BACKSCRIPT", "feed.php");
	define("FEED_FRONTSCRIPT", "feed.js");
	define("FEED_STYLESCRIPT", "feed.css");
	define("FEED_SNIPPETPATH", "snippets/");
	
	// (request modes)
	define("FEED_DISPLAY_INTERFACE", "interface");
	
	// (output modes)
	define("FEED_OUTPUT_HTML", "html");
	
	# -------------------------------------------------------------------------------------------------------------------
	# (class) mgFeedManager, manages the widget modules
	class mgFeedManager extends mgModuleManager {
		# ---------------------------------------------------------------------------------------------------------------
		# (initialize) initialize the module manager class
		public function initialize(){
			// initialize module manager
			return Array(
				MODULE_CLASS=>FEED_CLASS,
				MODULE_PATH=>Array(FEED_PATH, SHARED_PATH.FEED_PATH),
				MODULE_XML=>FEED_XML,
				MODULE_RESOURCES=>Array(FEED_BACKSCRIPT, FEED_FRONTSCRIPT, FEED_STYLESCRIPT)
			);
		}
	...
	# -------------------------------------------------------------------------------------------------------------------
	# (class) mgFeed, manages a single widget
	class mgFeed extends mgModule {
		...
		public function loadmodule() {
			...
			if($this->asBool("registerjs", "value")===true) {
				...
				$this->manager->framework->resources->asjs($this->asResource(FEED_FRONTSCRIPT));
			}
			
			// create filename
			$fn = sprintf("%s/%s", dirname($this->location), FEED_BACKSCRIPT);
			// validate filename
			if(file_exists($fn)) {
				try {
					// load library
					require_once($fn);
					// create module name
					$name = sprintf("%s%s", FEED_CLASS, ucfirst($this->name));
		...
		public function output($target=false, $settings=false, $values=false, $baseurl=false) {
			...
			try {
				...
				if($this->script) {
					...
					if($result!==false) {
						// add css if any
						$this->manager->framework->resources->ascss($this->asResource(FEED_STYLESCRIPT));
					}
				}
				// set content if any
			} catch(Exception $e) {
		...
		public function getdisplay($type = false) {
			...
			switch($type) {
				# -------------------------------------------------------------------------------------------------------
				# (interface)
				case FEED_DISPLAY_INTERFACE:
					// create data array
					return Array(
						"feedid"=>$this->id,
						"feedenabled"=>$this->asBool("settings", "enabled"),
						"feedname"=>$this->name,
						"feedscript"=>$this->asScript(FEED_FRONTSCRIPT),
					);				
					break;
			}
		}
	...
	# -------------------------------------------------------------------------------------------------------------------
	# (class) mgFeedExtension 
	class mgFeedExtension extends mgAdopterClass {
		...
		public function __get($name) {
			...
			switch($name) {
				...
				case "urlpath": return sprintf(FEED_URLPATH, $this->id); break;
				case "gateway": return sprintf(FEED_GATEWAY, $this->urlpath); break;
				case "status": return $this->read("status", FEED_STATUS_DISCONNECTED); break;
			}
			// acquire parent
		...
		public function snippet($name) {
			// create filename
			$fn = sprintf("%s/%s%s.snippet", dirname($this->location), FEED_SNIPPETPATH, $name);
			// check if exists
