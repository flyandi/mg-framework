/**
  ...
  *
  * Module 		mgDateTimePicker Control
  */

  
var mgDateTimePicker =
{
  	//=============================================================================
  	//  mgDateTimePicker.pad() pads a value with a specified number of zeroes and returns
...
(function($)
{
	...
	var __msie7 = ( navigator.userAgent.indexOf('MSIE 7') > 0 ); 
  	...
  	jQuery.prototype.mgDateTimePicker_picker = function( options )
  	{
  		return this.each( function(i) { mgDateTimePicker.picker( $(this), options ); } );
  	}
  	
  	jQuery.prototype.mgDateTimePicker_noPicker = function()
  	{
  		return this.each( function(i) { mgDateTimePicker.noPicker( this.id ); } );
  	}
	...
	//  This prevents the pickers from being too tall and wide.
  	...
  	jQuery.prototype.mgDateTimePicker_height = function(inclusive)
  	{
  		return ( __msie6 ?
  	...
  	jQuery.prototype.mgDateTimePicker_width = function(inclusive)
  	{
  		return ( __msie6 ?
  	...
  	//  indicate whether it's value is current (used by mgDateTimePicker.picker),
  	//  and another to trigger the click handler for the currently-
  	//  selected button under an element.

  	jQuery.prototype.mgDateTimePicker_current = function(isCurrent,isLegal)
	{
	    if ( isCurrent )
	    {
		  this.removeClass('mgDateTimePicker-out-btn ui-state-default ui-state-disabled ui-state-highlight');
	      this.addClass('mgDateTimePicker-cur-btn ui-state-default ui-state-highlight');
	    }
	    else
	    {
	      this.removeClass('mgDateTimePicker-cur-btn ui-state-highlight');
	      if ( ! isLegal )
	    	  this.addClass('mgDateTimePicker-out-btn ui-state-disabled');
	      else
	    	  this.removeClass('mgDateTimePicker-out-btn ui-state-disabled');
	    }
	};
	
	jQuery.prototype.mgDateTimePicker_clickCurrent = function()
	{
		this.find('.mgDateTimePicker-cur-btn').triggerHandler('click');
	}
...
//=============================================================================
//  mgDateTimePicker.Converter
//
//  This object converts between Date objects and Strings.
//
//  To use mgDateTimePicker.Converter, simply create an instance for a format string,
//  and then (repeatedly) invoke the format() and/or parse() methods to
//  perform the conversions.  For example:
//
//    var converter = new mgDateTimePicker.Converter({format:'%Y-%m-%d'})
...
//      specifier is used.  By default, mgDateTimePicker.Converter follows the
...
//      Note that if the firstDOW option is passed to mgDateTimePicker.picker() (see
//      mgDateTimePicker.picker()), this array should nonetheless begin with the 
...
//          mgDateTimePickertz.js file), then it is assumed to be an array of arrays,
...
//      This primarily exists so mgDateTimePicker.picker can edit the time as specified
...
//
//    utcFormatOffsetSubIndex - when extending mgDateTimePicker with a utcLabel array
//      (for example, by the mgDateTimePickertz.js file), the specified sub-index is
//      used to choose the Time Zone label for the UTC offset when formatting
//      a Date object.  This primarily exists so mgDateTimePicker.picker can specify
...
//      mgDateTimePicker.picker can edit the time as specified (without conversion to
//      local time) and then convert the edited time to a different time zone
//      (as selected using the picker). 
//=============================================================================

mgDateTimePicker.Converter = function(options)
{
  	// private members
	...
	//-------------------------------------------------------------------------
	//  mgDateTimePicker.Converter.dAt() returns true if the character in str at pos
	//  is a digit.
	//-------------------------------------------------------------------------
	...
	//-------------------------------------------------------------------------
	//  mgDateTimePicker.Converter.format() returns a String containing the value
	      ...
	      else
	      {
	        ...
	        switch ( ch )
	        {
	          ...
	          case 'Y': // Year, numeric, four digits (negative if before 0001)
	            str += mgDateTimePicker.pad(d.getFullYear(),4);
	            break;
	          case 'y': // Year, numeric (two digits, negative if before 0001)
	            t = d.getFullYear() % 100;
	            str += mgDateTimePicker.pad(t,2);
	            break;
	          case 'Z': // Year, numeric, four digits, unsigned (NON-MYSQL)
	            str += mgDateTimePicker.pad(Math.abs(d.getFullYear()),4);
	            break;
	          ...
	          case '@': // timezone offset label
		        t = ( _offAl != Number.MIN_VALUE ) ? _offAl :
		        	( _offF == Number.MIN_VALUE ) ? (0-d.getTimezoneOffset()) : _offF;
		        if ( mgDateTimePicker.utcLabel && mgDateTimePicker.utcLabel[t] )
		        {
		          if ( ( _offFSI > 0 ) && ( _offFSI < mgDateTimePicker.utcLabel[t].length ) )
		            str += mgDateTimePicker.utcLabel[t][_offFSI];
		          else
		            str += mgDateTimePicker.utcLabel[t][0];
		          break;
		        }
		        str += 'UTC';
	          ...
	          case ';': // signed 3-or-4-digit timezone offset with colon delimiter
		        ...
		        if ( t < 0 )
		          ...
		          str += '+';
		        t = Math.abs(t);
		        str += ((ch=='+')||(ch==':')) ? mgDateTimePicker.pad(Math.floor(t/60),2) : Math.floor(t/60);
		        if ( (ch==':') || (ch==';') )
		          str += ':';
		        str += mgDateTimePicker.pad(t%60,2);
		        break;
	          ...
	          case 'x': // Year for the week, where Monday is the first day of the week, numeric, four digits; used with %v
	            throw '%'+ch+' not implemented by mgDateTimePicker.Converter';
	...
	}; // mgDateTimePicker.Converter.format()
		  
	//-------------------------------------------------------------------------
	//  mgDateTimePicker.Converter.getUtcParseOffsetCaptured() returns the UTC offset
	...
	//-------------------------------------------------------------------------
	//  mgDateTimePicker.Converter.parse() returns a Date initialized from a specified
	//  string, using the format passed to mgDateTimePicker.Converter().
	//
	//  Method parameter:
	//
	    ...
	    for ( var f = 0 ; f < _flen ; f++ )
	    {
	      if ( this.fmt.charAt(f) == '%' )
	      {
            ...
            case 'w': // Day of the week (0=Sunday..6=Saturday) (ignored)
	          ...
	          case '@': // timezone label
	        	_offPSI = (-1);
	        	if ( mgDateTimePicker.utcLabel )
	        	{
		            matched = false;
		            for ( tzOff in mgDateTimePicker.utcLabel )
                  if ( ! Array.prototype[tzOff] ) // prototype.js compatibility issue
		              {
		            	  for ( i = 0 ; i < mgDateTimePicker.utcLabel[tzOff].length ; i++ )
  		            	{
	  	            		sub = mgDateTimePicker.utcLabel[tzOff][i];
		              		sublen = sub.length;
	          ...
	          case 'X': // Year for the week where Sunday is the first day of the week, numeric, four digits; used with %V
	          case 'x': // Year for the week, where Monday is the first day of the week, numeric, four digits; used with %v
	            throw '%'+this.fmt.charAt(f+1)+' not implemented by mgDateTimePicker.Converter';
	          case '%': // A literal '%' character
	          default: // for any character not listed above
	...
	}; // mgDateTimePicker.Converter.parse()
	
	//-------------------------------------------------------------------------
	//  mgDateTimePicker.Converter.setUtcFormatOffsetAlleged()  sets the offset from 
	...
	//  affect the date/time value.  This primarily exists so mgDateTimePicker.picker
	...
	//-------------------------------------------------------------------------
	//  mgDateTimePicker.Converter.setUtcFormatOffsetSubIndex() sets the sub-index
	//  to choose from the mgDateTimePicker.utcLabel array of arrays when formatting
	//  a Date using the %@ specifier.  For more information, see option
	//  mgDateTimePicker.Converter.utcFormatOffsetSubIndex.  This primarily exists so
	//  mgDateTimePicker.picker can specify the Time Zone label selected using the
	...
	//-------------------------------------------------------------------------
	//	mgDateTimePicker.Converter construction code:
	//-------------------------------------------------------------------------
	...
	})(this); // mgDateTimePicker.Converter construction

}; // mgDateTimePicker.Converter = 

//=============================================================================
//  mgDateTimePicker.noPicker()
//
//  Removes the date/time entry picker attached to a specified text field.
//=============================================================================

mgDateTimePicker.noPicker = function( id )
{
	if ( __pickers[id] )
	{
...
//
//    <link rel="stylesheet" type="text/css" href="mgDateTimePicker.css" />
//    <script type="text/javascript" src="jquery.js"></script>
//    <script type="text/javascript" src="mgDateTimePicker.js"></script>
//    <input type="text" id="foo" tabindex="1" value="1967-07-30 23:45" />
//    <input type="text" id="bar" tabindex="2" value="01/06/90" />
//    <script type="text/javascript">
//      mgDateTimePicker.picker( "foo" );
//      mgDateTimePicker.picker( "bar", { placement:"inline", format: "%m/%d/%y",
//								ajaxOptions { url: "/some/server/page/" } } );
//    </script>
//
//  The appearance of the picker can be extensively modified using CSS styles.
//  A default appearance can be achieved by the "mgDateTimePicker.css" stylesheet that
...
//    mgDateTimePicker.picker object.  The mgDateTimePicker.picker will attach itself
...
//        format (see mgDateTimePicker.Converter.format()).
//
...
//      if custom arrays are specified for mgDateTimePicker.Converter's dayAbbreviations
...
//    for mgDateTimePicker.Converter (above) for information about these options:
//
//      baseYear
//      dayAbbreviations
//      dayNames
...
mgDateTimePicker.picker = function( target , options )
{
    ...
    if ( __pickers[id] )
    	throw 'Cannot create another mgDateTimePicker picker for "'+id+'"';
	...
	__pickers[id] =
	{
		...
		conv: null,			// mgDateTimePicker.Converter
		...
		oConv: null,        // mgDateTimePicker.Converter for offset display
		...
		initialize: function( id )
		{
			...
			target.data('__ref', this);
			

			this.id = 'mgDateTimePicker--'+mgUniqueId();
			options = jQuery.extend(true,{},options||{});
		  	options.utcParseOffsetCapture = true;
		  	this.conv = new mgDateTimePicker.Converter(options);
		  	...
		  	if ( askOff )
			  	this.oConv = new mgDateTimePicker.Converter( { format: options.formatUtcOffset || 
			...
			target.data("dtpickerrel", this.id);

			this.inp = target;
		  	this.div = $( '<div class="mgDateTimePicker-win mgDateTimePicker-pkr ui-widget ui-widget-content ui-corner-all" style="width:0;height:0" id="' + this.id + '" aria-live="off"/>' );
		    this.inp.after(this.div);
		  	this.wMinW = this.div.outerWidth(!$.browser.safari);
		  	this.wMinH = this.div.mgDateTimePicker_height(true);
		  	this.hTitle = $( '<h5 class="mgDateTimePicker-hdr ui-widget-header ui-corner-top"/>' ); 
		  	this.div.append( this.hTitle );
		  	this.dB = $( '<div class="mgDateTimePicker-body" style="width:0;height:0"/>' );
		  	this.div.append( this.dB );
		  	this.bMinW = this.dB.outerWidth(true);
		  	this.bMinH = this.dB.mgDateTimePicker_height(true);
		  	...
		  	if ( this.pop )
		  	{
		  		xDiv = $( '<div class="mgDateTimePicker-x-btn ui-state-default">'+this.lX+'</div>' );
		  	...
		  	if ( askDate )
		  	{
			  this.dD = $( '<div class="mgDateTimePicker-date" style="width:0;height:0"/>' );
			  this.dB.append( this.dD );
		  	  this.dMinW = this.dD.outerWidth(true);
		  	  this.dMinH = this.dD.mgDateTimePicker_height(true);

		      if ( askYear )
		      {
		    	  this.yLab = $('<h6 class="mgDateTimePicker-lbl mgDateTimePicker-lbl-yr">' + this.lY + '</h6>');
		    	  this.dD.append( this.yLab );

		          this.dY = $( '<ul class="mgDateTimePicker-yrs ui-helper-reset" />' );
		          ...
		          this.yCur.addClass('mgDateTimePicker-cur-btn ui-state-default ui-state-highlight');
		      ...
		      if ( askMonth )
		      {
		    	  ...
		    	  this.hMo = $( '<h6 class="mgDateTimePicker-lbl mgDateTimePicker-lbl-month">' + lab + '</h6>' );
		    	  this.dD.append( this.hMo );
		    	  this.dMo = $('<ul class="mgDateTimePicker-mons" />');
		    	  ...
		    	  for ( i = 0 ; i < 12 ; i++ )
		    	  {
		    		  var mBtn = this.btn( this.dMo, this.conv.mAbbr[i], 
			    			function( event )
			    			{
			    			    ...
			    			    if ( elem.hasClass("mgDateTimePicker-out-btn") )
			    			    	return;
			    				var mo = event.target.mgDateTimePicker_month;
		    		  ...
		    		  mBtn[0].mgDateTimePicker_month = i;
		    	  }
		    	  shownFields++;
		      }
		      ...
		      if ( askDoM )
		      {
		        ...
		        this.hDoM = $('<h6 class="mgDateTimePicker-lbl mgDateTimePicker-lbl-dom">' + lab + '</h6>' );
		      	this.dD.append( this.hDoM );
		        this.dDoM =  $( '<table border="0" cellpadding="0" cellspacing="0" class="mgDateTimePicker-dom-table"/>' );
		        this.dD.append( this.dDoM );
		        t = $( '<thead class="mgDateTimePicker-dom-head"/>' );
		        this.dDoM.append(t);
		        var tr = $( '<tr class="mgDateTimePicker-dow"/>' );
		        t.append(tr);
		        for ( i = 0 ; i < 7 ; i++ )
		          tr.append( '<th class="mgDateTimePicker-dow mgDateTimePicker-dow'+String(i+1)+'">'+this.conv.dAbbr[(this.fDOW+i)%7]+'</th>' );

		        var tbody = $( '<tbody class="mgDateTimePicker-dom-body" />' );
		        this.dDoM.append(tbody);
		        for ( var r = 0 ; r < 6 ; r++ )
		        {
		          tr = $( '<tr class="mgDateTimePicker-wk mgDateTimePicker-wk'+String(r+1)+'"/>' );
		          tbody.append(tr);
		          for ( i = 0 ; i < 7 ; i++ )
		        	  this.btn( tr, 'x',
		        		function( event )
          ...
          var tensDiv, onesDiv;
		      ...
		      this.dT = $('<div class="mgDateTimePicker-time" style="width:0;height:0" />');
		      this.dB.append(this.dT);
		  	  this.tMinW = this.dT.outerWidth(true);
		  	  this.tMinH = this.dT.mgDateTimePicker_height(true);

		      if ( askHour )
		      {
		        this.dH = $('<div class="mgDateTimePicker-hrs"/>');
		        this.dT.append(this.dH);

		        lab = options.labelHour || 'Hour';
		        this.dH.append( $('<h6 class="mgDateTimePicker-lbl mgDateTimePicker-lbl-hr">'+lab+'</h6>') );
		        var amDiv = $('<ul class="mgDateTimePicker-hrs-am"/>');
		        this.dH.append( amDiv );
		        var pmDiv = $('<ul class="mgDateTimePicker-hrs-pm"/>');
		      ...
		      if ( askMinute )
		      {
		        this.dM = $('<div class="mgDateTimePicker-mins"/>');
		        this.dT.append(this.dM);

		        lab = options.labelMinute || 'Minute';
		        this.dM.append( $('<h6 class="mgDateTimePicker-lbl mgDateTimePicker-lbl-min">'+lab+'</h6>') );
            tensDiv = $('<ul class="mgDateTimePicker-mins-tens"/>');
		        ...
		        for ( ; i < 12 ; i++ )
			          this.btn( tensDiv, '&#160;', $.noop, ['min-ten','min'+i+'0'], lab+' '+i+'0' ).addClass('mgDateTimePicker-min-ten-btn-empty ui-state-default ui-state-disabled');

            onesDiv = $('<ul class="mgDateTimePicker-mins-ones"/>');
		        ...
		        for ( i = 0 ; i < 10 ; i++ )
		          this.btn( onesDiv, i, 
		    		  ...
		    		  ['min-one','min'+i], lab+' '+i );
		        for ( ; i < 12 ; i++ )
			          this.btn( onesDiv, '&#160;', $.noop, ['min-one','min'+i+'0'], lab+' '+i ).addClass('mgDateTimePicker-min-one-btn-empty ui-state-default ui-state-disabled');
		      ...
		      if ( askSec )
		      {
		        this.dS = $('<div class="mgDateTimePicker-secs"/>');
		        this.dT.append(this.dS);
		        lab = options.labelSecond || 'Second';
		        this.dS.append( $('<h6 class="mgDateTimePicker-lbl mgDateTimePicker-lbl-sec">'+lab+'</h6>') );
            tensDiv = $('<ul class="mgDateTimePicker-secs-tens"/>');
		        ...
		        for ( ; i < 12 ; i++ )
			          this.btn( tensDiv, '&#160;', $.noop, ['sec-ten','sec'+i+'0'], lab+' '+i+'0' ).addClass('mgDateTimePicker-sec-ten-btn-empty ui-state-default ui-state-disabled');

            onesDiv = $('<ul class="mgDateTimePicker-secs-ones"/>');
		        ...
		        for ( i = 0 ; i < 10 ; i++ )
		          this.btn( onesDiv, i,
		    		  ...
		    		  ['sec-one','sec'+i], lab+' '+i );
		        for ( ; i < 12 ; i++ )
			          this.btn( onesDiv, '&#160;', $.noop, ['sec-one','sec'+i+'0'], lab+' '+i ).addClass('mgDateTimePicker-sec-one-btn-empty ui-state-default ui-state-disabled');
		      ...
		      if ( askOff )
		      {
		    	this.dO = $('<div class="mgDateTimePicker-offs" />');
		        this.dT.append(this.dO);
			  	this.oMinW = this.dO.outerWidth(true);
		        
		    	this.oLab = $('<h6 class="mgDateTimePicker-lbl mgDateTimePicker-lbl-off">' + this.lO + '</h6>');
		    	this.dO.append( this.oLab );
		        
		    	var offDiv = $('<ul class="mgDateTimePicker-off-list ui-helper-reset" />');
		        this.dO.append(offDiv);
		        
		        this.oCur = this.btn(offDiv,'',this.newOffset,['off','off-cur'],lab);
		        this.oCur.removeClass('ui-state-default');
		        this.oCur.addClass('mgDateTimePicker-cur-btn ui-state-default ui-state-highlight');
		    ...
		    try	{
			  if(options.timestamp){
				this.time = mgUnixToDateTime(options.timestamp);
				if(this.time) {
					this.set(this.time);
					this.upd(null);
					this.inp.removeClass("isdefault");
				} else {
					this.time = new Date();
				}
			  } else {
		...
		askOffset: function( event )
		{
		    if ( ! this.oDiv )
		    {
		      ...
		      this.oDiv = $('<div class="mgDateTimePicker-win mgDateTimePicker-off-selector ui-widget ui-widget-content ui-corner-all" style="position:absolute" />');
		      this.div.append(this.oDiv);
		
		      // the order here (HDR,BODY,XDIV,TITLE) is important for width calcluation:
		      var title = $('<h5 class="mgDateTimePicker-hdr mgDateTimePicker-hdr-off-selector ui-widget-header ui-corner-top" />');
		      this.oDiv.append( title );
		      this.oBody = $('<div class="mgDateTimePicker-body mgDateTimePicker-body-off-selector" style="overflow:auto;white-space:nowrap" />');
		      this.oDiv.append( this.oBody );
		      var oBHS = this.oBody.mgDateTimePicker_height(true); // body spacing
		      var oBWS = this.oBody.mgDateTimePicker_width(true);
		      var oTWS = title.mgDateTimePicker_width(true);
		      
		      var xDiv = $('<div class="mgDateTimePicker-x-btn ui-state-default">'+this.lX+'</div>');
		      title.append(xDiv);
			  ...
			  var oBW = title.mgDateTimePicker_width(true) - oBWS; // initial body width
		      
		      var cont = $('<ul class="mgDateTimePicker-off-off" />' );
		      var last = null;
		      ...
		      if ( last )
		    	  last.addClass('mgDateTimePicker-off-off-last-btn');
		      
		      // compute optimal width
		      
		      this.oBody.find('.mgDateTimePicker-off-off-btn').width(btnW); // set uniform button width
		      if ( last )
		      {
		          var lW = last.mgDateTimePicker_width(true);
		          if ( lW > oBW )
			          oBW = lW+1; // expand body to hold buttons
		      }
		      this.oBody.width(oBW);
		      oBW = this.oBody.mgDateTimePicker_width(true);
		      this.oDiv.width( oBW );		      
		      if ( __msie6 || __msie7 ) // IE bugs!
		      ...
		      var oH = this.oDiv.mgDateTimePicker_height(true);
		      var oHmax = this.div.height() * 0.75;
		      if ( oH > oHmax )
		      {
		    	  oH = oHmax;
		    	  this.oBody.height(oH-(title.mgDateTimePicker_height(true)+oBHS));
			      this.oBody.width(this.oBody.width()+20); // add nominal px for scrollbar 
			      this.oDiv.width(this.oDiv.width()+20); 
			      if ( __msie6 || __msie7 ) // IE bugs!
					  title.width( this.oBody.mgDateTimePicker_width(true) - oTWS );
		      }
			  if ( ! __msie7 ) // IE7 bug!
		    ...
		    var f = this.oDiv.find('.mgDateTimePicker-off-off-btn.mgDateTimePicker-cur-btn:first');
		    if ( ! f.length )
		    	f = this.oDiv.find('.mgDateTimePicker-off-off-btn:first');
		    this.setFocus( f );
		...
		askYear: function( event )
		{
		    if ( ! this.yDiv )
		    {
		      ...
		      this.yDiv = $('<div class="mgDateTimePicker-win mgDateTimePicker-yr-selector ui-widget ui-widget-content ui-corner-all" style="position:absolute" />');
		      this.div.append(this.yDiv);
		
		      var title = $('<h5 class="mgDateTimePicker-hdr mgDateTimePicker-hdr-yr-selector ui-widget-header ui-corner-top" />');
		      ...
		      var yBody = $('<div class="mgDateTimePicker-body mgDateTimePicker-body-yr-selector" />');
		      var yW = yBody.mgDateTimePicker_width(true);
		      var yH = 0;
		      this.yDiv.append( yBody );
		      
		      cont = $('<ul class="mgDateTimePicker-yr-mil" />' );
		      yBody.append(cont);
		      this.y0XXX = this.btn( cont, 0, this.newYPos,['mil','mil0'],this.lY+' '+0+'000');
		      for ( i = 1; i < 10 ; i++ )
		        this.btn( cont, i, this.newYPos,['mil','mil'+i],this.lY+' '+i+'000');
		      yW += cont.mgDateTimePicker_width(true);
		      if ( yH < cont.mgDateTimePicker_height(true) )
		    	  yH = cont.mgDateTimePicker_height(true);
		
			  cont = $('<ul class="mgDateTimePicker-yr-cent" />' );
		      yBody.append(cont);
		      for ( i = 0 ; i < 10 ; i++ )
		      ...
		      cont = $('<ul class="mgDateTimePicker-yr-dec" />');
		      yBody.append(cont);
		      for ( i = 0 ; i < 10 ; i++ )
		      ...
		      cont = $('<ul class="mgDateTimePicker-yr-yr" />');
		      yBody.append(cont);
		      for ( i = 0 ; i < 10 ; i++ )
		      ...
		      if ( this.askEra )
		      {
		        cont = $('<ul class="mgDateTimePicker-yr-era" />' );
		        yBody.append(cont);
		      ...
		      if ( $.browser.msie ) // IE8+ThemeUI bug!
		    	  ...
		    	  yW += 2;
			  yH += yBody.mgDateTimePicker_height(true);
			  yBody.css('width',String(yW)+'px');
			  if ( ! __msie7 ) // IE7 bug!
				  yBody.css('height',String(yH)+'px');
		      if ( __msie6 || __msie7 ) // IE bugs!
		    	  title.width(yBody.outerWidth(true));
		      yH += title.mgDateTimePicker_height(true);
		      if ( title.mgDateTimePicker_width(true) > yW )
		          yW = title.mgDateTimePicker_width(true);
		    ...
		    this.pos(event);
		    this.updYDiv(null);
		    this.setFocus( this.yDiv.find('.mgDateTimePicker-yr-btn.mgDateTimePicker-cur-btn:first') );
		...
		inpBlur: function(event)
		{
		    ...
		    setTimeout(
		    	...
		    	{ 
		    		if ( _this.lostFocus )
		    		{
		    			_this.div.find('.mgDateTimePicker-focus-btn').removeClass('mgDateTimePicker-focus-btn ui-state-focus'); 
		    			if ( _this.pop )
		    				_this.dismiss(event);
		    			else
		    				_this.ajax();
		...
		//---------------------------------------------------------------------
		//  .btn() is called by mgDateTimePicker.picker() to create a <div> element
		...
		btn: function( parent, text, handler, classes, title )
		{
			...
			var div$ = '<' + tagName +
			  				' class="mgDateTimePicker-btn';
			for ( var i = 0 ; i < classes.length ; i++ )
				div$ += ' mgDateTimePicker-' + classes[i] + '-btn';
			var div = $( div$ + ' ui-state-default">' + text + '</' + tagName + '>' );
			parent.append(div);
			div.mgDateTimePicker_title = title;
			...
			div.dblclick(
				...
				{ 					
					var elem = $(this);
					if ( elem.is('.mgDateTimePicker-off-off-btn') )
						_this.dismissODiv(e);
					else if ( elem.is('.mgDateTimePicker-mil-btn') || elem.is('.mgDateTimePicker-cent-btn') || elem.is('.mgDateTimePicker-dec-btn') || elem.is('.mgDateTimePicker-yr-btn') || elem.is('.mgDateTimePicker-era-btn') )
						_this.dismissYDiv(e);
					else if ( _this.pop )
						_this.dismiss(e); 
				});
		...
		{
			if ( ! btn.hasClass('mgDateTimePicker-focus-btn') )
			{
				this.div.find('.mgDateTimePicker-focus-btn').removeClass('mgDateTimePicker-focus-btn ui-state-focus');
				this.fBtn = btn;
				btn.removeClass('ui-state-default ui-state-highlight');
				btn.addClass('mgDateTimePicker-focus-btn ui-state-default ui-state-highlight ui-state-focus');
			}
			if ( btn.hasClass('mgDateTimePicker-off-off-btn') )
			{
				var oBT = this.oBody.offset().top;
				var btnT = btn.offset().top;
				var btnH = btn.mgDateTimePicker_height(true);
		...
		//  but mgDateTimePicker.picker uses the following behavior to be as consistent
		...
		//
		//  Because mgDateTimePicker.picker sets a value as soon as the button receives
      ...
      var mo;
			var t = null;
			var elem = this.div.find('.mgDateTimePicker-focus-btn');
		    ...
		    {
		    	if ( this.fBtn.hasClass('mgDateTimePicker-off-off-btn') )
				{
		    		if ( key == 9 )
				        this.dismissODiv(event);
				}
		    	else if ( this.fBtn.hasClass('mgDateTimePicker-mil-btn') )
				{
		    		if ( key == 9 )
				        this.dismissYDiv(event);
				}
		    	else if ( this.fBtn.hasClass('mgDateTimePicker-cent-btn') )
					this.yDiv.find('.mgDateTimePicker-mil-btn.mgDateTimePicker-cur-btn').triggerHandler('click');
		    	else if ( this.fBtn.hasClass('mgDateTimePicker-dec-btn') )
					this.yDiv.find('.mgDateTimePicker-cent-btn.mgDateTimePicker-cur-btn').triggerHandler('click');
		    	else if ( this.fBtn.hasClass('mgDateTimePicker-yr-btn') )
					this.yDiv.find('.mgDateTimePicker-dec-btn.mgDateTimePicker-cur-btn').triggerHandler('click');
		    	else if ( this.fBtn.hasClass('mgDateTimePicker-era-btn') )
					this.yDiv.find('.mgDateTimePicker-yr-btn.mgDateTimePicker-cur-btn').triggerHandler('click');
		    	else if ( this.fBtn.parents('.mgDateTimePicker-yrs').length )
				{
					if ( key == 9 )
					{
						this.denyTab = false;
		    	...
		    	else if ( this.fBtn.hasClass('mgDateTimePicker-mon-btn') )
				{
					if ( this.dY )
		    	...
		    	else if ( this.fBtn.hasClass('mgDateTimePicker-dom-btn') )
		    	{
		    	...
		    	else if ( this.fBtn.hasClass('mgDateTimePicker-hr-btn') )
				{
		    		t = this.dDoM || this.dMo;
					if ( t )
						t.mgDateTimePicker_clickCurrent();
					else if ( this.dY )
						this.yCur.triggerHandler('click');
					else if ( key == 9 )
					{
		    	...
		    	else if ( this.fBtn.hasClass('mgDateTimePicker-min-ten-btn') )
				{
		    	...
		    	else if ( this.fBtn.hasClass('mgDateTimePicker-min-one-btn') )
					this.dM.mgDateTimePicker_clickCurrent();
		    	else if ( this.fBtn.hasClass('mgDateTimePicker-sec-ten-btn') )
				{
		    		if ( this.dM )
		    			t = this.dM.find('.mgDateTimePicker-mins-ones');
		    		else
		    			t = this.dH || this.dDoM || this.dMo;
					if ( t )
						t.mgDateTimePicker_clickCurrent();
		    	...
		    	else if ( this.fBtn.hasClass('mgDateTimePicker-sec-one-btn') )
					this.dS.mgDateTimePicker_clickCurrent();
		    	else if ( this.fBtn.hasClass('mgDateTimePicker-off-btn') )
				{
		    		if ( this.dS )
		    			t = this.dS.find('.mgDateTimePicker-secs-ones');
		    		else if ( this.dM )
		    			t = this.dM.find('.mgDateTimePicker-mins-ones');
		    		else
		    			t = this.dH || this.dDoM || this.dMo;
		    ...
		    {
		    	if ( this.fBtn.hasClass('mgDateTimePicker-mil-btn') )
					this.yDiv.find('.mgDateTimePicker-cent-btn.mgDateTimePicker-cur-btn').triggerHandler('click');
		    	else if ( this.fBtn.hasClass('mgDateTimePicker-cent-btn') )
					this.yDiv.find('.mgDateTimePicker-dec-btn.mgDateTimePicker-cur-btn').triggerHandler('click');
		    	else if ( this.fBtn.hasClass('mgDateTimePicker-dec-btn') )
		    	...
		    	{
		    		t = this.yDiv.find('.mgDateTimePicker-era-btn.mgDateTimePicker-cur-btn');
					if ( t.length )
		    	...
		    	else if ( this.fBtn.hasClass('mgDateTimePicker-off-off-btn') )
		    	{
		    		if ( key == 9 )
		    			this.dismissODiv(event);
		    	}
		    	...
		    	else if ( this.fBtn.hasClass('mgDateTimePicker-hr-btn') )
				{
		    		...
		    		this.dM.find('.mgDateTimePicker-mins-ones .mgDateTimePicker-cur-btn').triggerHandler('click');
		    	else if ( this.fBtn.hasClass('mgDateTimePicker-min-one-btn') )
				{
					t = this.dS || this.dO;
					if ( t )
		    		...
		    		this.dS.find('.mgDateTimePicker-secs-ones .mgDateTimePicker-cur-btn').triggerHandler('click');
		    	else if ( this.fBtn.hasClass('mgDateTimePicker-sec-one-btn') )
				{
		    		if ( this.dO )
						this.dO.mgDateTimePicker_clickCurrent();
		    		else if ( key == 9 )
					{
						this.denyTab = false;
						return;
		    ...
		    {
		    	if ( this.fBtn.hasClass('mgDateTimePicker-mil-btn') || this.fBtn.hasClass('mgDateTimePicker-cent-btn') ||
				    this.fBtn.hasClass('mgDateTimePicker-dec-btn') || this.fBtn.hasClass('mgDateTimePicker-yr-btn') ||
				    this.fBtn.hasClass('mgDateTimePicker-era-btn') )
		    	{
		    		t = this.yDiv.find('.mgDateTimePicker-ce-btn');
		    		if ( ! t.length ) 
		    			t = this.yDiv.find('.mgDateTimePicker-yr9-btn');
		    		t.triggerHandler('click');
		    	}
		    	else if ( this.fBtn.hasClass('mgDateTimePicker-dom-btn') )
		    	{
					...
					this.dS.find('.mgDateTimePicker-sec9-btn').triggerHandler('click');
		    	else if ( this.dM )
					this.dM.find('.mgDateTimePicker-min9-btn').triggerHandler('click');
				else if ( this.dH )
					this.dH.find('.mgDateTimePicker-hr23-btn').triggerHandler('click');
				else if ( this.dDoM )
					this.dDoM.find('.mgDateTimePicker-dom-btn-filled:last').triggerHandler('click');
				else if ( this.dMo )
					this.dMo.find('.mgDateTimePicker-mon12-btn').triggerHandler('click');
				else if ( this.dY )
		    ...
		    else if ( key == 36 ) // HOME
		    {
		    	if ( this.fBtn.hasClass('mgDateTimePicker-mil-btn') || this.fBtn.hasClass('mgDateTimePicker-cent-btn') ||
				    ...
				    this.fBtn.hasClass('mgDateTimePicker-era-btn') )
				{
		    		this.yDiv.find('.mgDateTimePicker-mil0-btn').triggerHandler('click');
		    	}
			    else if ( this.fBtn.hasClass('mgDateTimePicker-dom-btn') )
		    	{
		    		t = new Date(this.time.getTime());
					...
					this.dMo.find('.mgDateTimePicker-mon1-btn').triggerHandler('click');
				else if ( this.dDoM )
					this.dDoM.find('.mgDateTimePicker-dom-btn-filled:first').triggerHandler('click');
				else if ( this.dH )
					this.dH.find('.mgDateTimePicker-hr0-btn').triggerHandler('click');
		    	else if ( this.dM )
					this.dM.find('.mgDateTimePicker-min00-btn').triggerHandler('click');
		    	else if ( this.dS )
					this.dS.find('.mgDateTimePicker-sec00-btn').triggerHandler('click');
		    }
		    else if ( key == 37 ) // left arrow
		    {
		    	if ( this.fBtn.hasClass('mgDateTimePicker-dom-btn') )
		...
		keyAhead: function()
		{
		   	if ( this.fBtn.hasClass('mgDateTimePicker-mil9-btn') )
		   		this.yDiv.find('.mgDateTimePicker-cent0-btn').triggerHandler('click');
		   	else if ( this.fBtn.hasClass('mgDateTimePicker-cent9-btn') )
		   		this.yDiv.find('.mgDateTimePicker-dec0-btn').triggerHandler('click');
		   	else if ( this.fBtn.hasClass('mgDateTimePicker-dec9-btn') )
		   		this.yDiv.find('.mgDateTimePicker-yr0-btn').triggerHandler('click');
		   	else if ( this.fBtn.hasClass('mgDateTimePicker-yr9-btn') )
		   		this.yDiv.find('.mgDateTimePicker-bce-btn').triggerHandler('click');
		   	else if ( this.fBtn.hasClass('mgDateTimePicker-sec9-btn') )
		   		{}
		   	else if ( this.fBtn.hasClass('mgDateTimePicker-sec50-btn') )
		   		this.dS.find('.mgDateTimePicker-sec0-btn').triggerHandler('click');
		   	else if ( this.fBtn.hasClass('mgDateTimePicker-min9-btn') )
		   	{
		   		if ( this.dS )
		   			this.dS.find('.mgDateTimePicker-sec00-btn').triggerHandler('click');
		   	}
		   	else if ( this.fBtn.hasClass('mgDateTimePicker-min50-btn') )
		   		this.dM.find('.mgDateTimePicker-min0-btn').triggerHandler('click');
		   	else if ( this.fBtn.hasClass('mgDateTimePicker-hr23-btn') )
		   	{
		   		if ( this.dM )
		   			this.dM.find('.mgDateTimePicker-min00-btn').triggerHandler('click');
		   		else if ( this.dS )
		   	...
		   	else if ( this.fBtn.hasClass('mgDateTimePicker-hr11-btn') )
				this.dH.find('.mgDateTimePicker-hr12-btn').triggerHandler('click');
		   	else if ( this.fBtn.hasClass('mgDateTimePicker-mon12-btn') )
		   	{
		   		if ( this.dDoM )
		   			this.dDoM.mgDateTimePicker_clickCurrent();
		   		else if ( this.dH )
		   			this.dH.find('.mgDateTimePicker-hr0-btn').triggerHandler('click');
		   		else if ( this.dM )
		   			this.dM.find('.mgDateTimePicker-min00-btn').triggerHandler('click');
		   	...
		   	else if ( this.fBtn.hasClass('mgDateTimePicker-yrs-ahead-btn') )
		   	{
		   		if ( this.dMo )
		   			this.dMo.find('.mgDateTimePicker-mon1-btn').triggerHandler('click');
		   		else if ( this.dH )
		   	...
		   	else if ( this.fBtn.hasClass('mgDateTimePicker-yr-cur-btn') )
		...
		keyBack: function()
		{
		   	if ( this.fBtn.hasClass('mgDateTimePicker-cent0-btn') )
		   		this.yDiv.find('.mgDateTimePicker-mil9-btn').triggerHandler('click');
		   	else if ( this.fBtn.hasClass('mgDateTimePicker-dec0-btn') )
		   		this.yDiv.find('.mgDateTimePicker-cent9-btn').triggerHandler('click');
		   	else if ( this.fBtn.hasClass('mgDateTimePicker-yr0-btn') )
		   		this.yDiv.find('.mgDateTimePicker-dec9-btn').triggerHandler('click');
		   	else if ( this.fBtn.hasClass('mgDateTimePicker-bce-btn') )
			   		this.yDiv.find('.mgDateTimePicker-yr9-btn').triggerHandler('click');
		   	else if ( this.fBtn.hasClass('mgDateTimePicker-yr-cur-btn') )
		        this.yPrior.triggerHandler('click');
		   	else if ( this.fBtn.hasClass('mgDateTimePicker-mon1-btn') )
		   	{
		   		if ( this.dY )
		   			this.yCur.triggerHandler('click');
		   	}
		   	else if ( this.fBtn.hasClass('mgDateTimePicker-hr0-btn') )
		   	{
		   		if ( this.dDoM )
		   			this.dDoM.mgDateTimePicker_clickCurrent();
		   		else if ( this.dMo )
		   	...
		   	else if ( this.fBtn.hasClass('mgDateTimePicker-hr12-btn') )
		   		 this.dH.find('.mgDateTimePicker-hr11-btn').triggerHandler('click');
		   	else if ( this.fBtn.hasClass('mgDateTimePicker-min00-btn') )
		   	{
		   		if ( this.dH )
		   			this.dH.find('.mgDateTimePicker-hr23-btn').triggerHandler('click');
		   		else if ( this.dDoM )
		   	...
		   	else if ( this.fBtn.hasClass('mgDateTimePicker-min0-btn') )
		   		 this.dM.find('.mgDateTimePicker-min50-btn').triggerHandler('click');
		   	else if ( this.fBtn.hasClass('mgDateTimePicker-sec00-btn') )
		   	{
		   		if ( this.dM )
		   			this.dM.find('.mgDateTimePicker-min9-btn').triggerHandler('click');
		   		else if ( this.dH )
		   	...
		   	else if ( this.fBtn.hasClass('mgDateTimePicker-sec0-btn') )
		   		 this.dS.find('.mgDateTimePicker-sec50-btn').triggerHandler('click');
		   	else
		...
		keyDateChange: function( newDate )
		{
			if ( this.fBtn.hasClass('mgDateTimePicker-dom-btn') )
			{
				this.set(newDate);
				this.upd(null);
				this.setFocus( this.dDoM.find('.mgDateTimePicker-cur-btn') );
			}
		},
		...
		makeCloak: function()
		{
			if ( ! this.cloak )
			{
		      this.cloak = $('<div class="mgDateTimePicker-cloak" style="position:absolute" />');
		...
		{
		    var elem = $(event.target);
            this.offMin = elem[0].mgDateTimePicker_offMin;
            this.offSI = elem[0].mgDateTimePicker_offSI;
		    var t = new Date(this.time.getTime());
		    this.set(t);
		    this.updODiv(elem);
		...
		newYPos: function( event )
		{
		    ...
		    year = mgDateTimePicker.pad( year, 4 );
		    if ( elem.hasClass('mgDateTimePicker-mil-btn') )
		      year = elem.html() + year.substring(1,4);
		    else if ( elem.hasClass('mgDateTimePicker-cent-btn') )
		      year = year.substring(0,1) + elem.html() + year.substring(2,4);
		    else if ( elem.hasClass('mgDateTimePicker-dec-btn') )
		...
		pos: function(event) // note: event is ignored but this is a handler
		{
		    ...
		    if ( this.oDiv && this.oDiv.is(":visible") )
		    {
		      ...
		      var oW = this.oDiv.mgDateTimePicker_width(true);
		      var wW = this.div.mgDateTimePicker_width(true);
		      ...
		      var oH = this.oDiv.mgDateTimePicker_height(true);
		      var wH = this.div.mgDateTimePicker_height(true);
		      oOff.top += this.oLab.mgDateTimePicker_height(true);
		...
		showPkr: function(event)
		{
		    ...
		    fBtn = null;
		    var cb = '.mgDateTimePicker-cur-btn:first';
		    if ( this.dDoM )
		...
		upd: function(fBtn)
		{
			this.inp.data('timestamp', this.time);
		    ...
		    if ( this.earliest && this.yPast )
		    {
			      ...
			      if ( cmpHi.getTime() < this.earliest )
			    	  this.yPast.addClass('mgDateTimePicker-out-btn ui-state-disabled');
			      else
			    	  this.yPast.removeClass('mgDateTimePicker-out-btn ui-state-disabled');
		    }
		    if ( this.yPrior )
		    {
		      this.yPrior.text(mgDateTimePicker.pad((current==1)?(-1):(current-1),4));
		      if ( this.earliest )
		      {
				  cmpHi.setYear(current-1);
			      if ( cmpHi.getTime() < this.earliest )
			    	  this.yPrior.addClass('mgDateTimePicker-out-btn ui-state-disabled');
			      else
			    	  this.yPrior.removeClass('mgDateTimePicker-out-btn ui-state-disabled');
		      }
		    }
		    if ( this.yCur )
		      this.yCur.text(mgDateTimePicker.pad(current,4));
		    if ( this.yNext )
		    {
		      this.yNext.text(mgDateTimePicker.pad((current==-1)?1:(current+1),4));
		      if ( this.latest )
		      {
			      cmpLo.setYear(current+1);
			      if ( cmpLo.getTime() > this.latest )
			    	  this.yNext.addClass('mgDateTimePicker-out-btn ui-state-disabled');
			      else
			    	  this.yNext.removeClass('mgDateTimePicker-out-btn ui-state-disabled');
		      }
		    }
		    if ( this.latest && this.yAhead )
		    {
			      ...
			      if ( cmpLo.getTime() > this.latest )
			    	  this.yAhead.addClass('mgDateTimePicker-out-btn ui-state-disabled');
			      else
			    	  this.yAhead.removeClass('mgDateTimePicker-out-btn ui-state-disabled');
		    }
		    ...
		    $('#'+this.id+' .mgDateTimePicker-mon-btn').each(
		      function()
		      {
		    	cmpLo.setMonth(i);
				cmpHi.setDate(1);
		    	cmpHi.setMonth(i+1);
				cmpHi.setDate(0);
		        $(this).mgDateTimePicker_current( i == current,
		    ...
		    if ( this.fDOW > dow1 )
		      dow1 += 7;
		    var wom = 0, dow=0;
		    $('#'+this.id+' .mgDateTimePicker-wk').each(
		      function()
		      {
		        dow = _this.fDOW;
		        $(this).children().each(
		          function()
		          {
		        	  if ( dow - _this.fDOW < 7 )
		        	  {
				          ...
				          {
				            td.html('&#160;');
				            td.removeClass('mgDateTimePicker-dom-btn-filled mgDateTimePicker-cur-btn ui-state-default ui-state-highlight');
				            td.addClass('mgDateTimePicker-dom-btn-empty');
				            if ( wom ) // not first week
				            {
				            	if ( ( cmpLo.getDate() == 1 ) && ( dow != 0 ) )
				            		td.addClass('mgDateTimePicker-dom-btn-empty-after-filled');
				            	else
				            		td.removeClass('mgDateTimePicker-dom-btn-empty-after-filled');
				            	if ( cmpLo.getDate() <= 7 )
				            		td.addClass('mgDateTimePicker-dom-btn-empty-below-filled');
				            	else
				            		td.removeClass('mgDateTimePicker-dom-btn-empty-below-filled');
				                cmpLo.setDate(cmpLo.getDate()+1);
				                cmpHi.setDate(cmpHi.getDate()+1);
				            }
				            else // first week
				            {
				            	td.addClass('mgDateTimePicker-dom-btn-empty-above-filled');
				            	if ( dow == dow1 - 1 )
				            		td.addClass('mgDateTimePicker-dom-btn-empty-before-filled');
				            	else
				            		td.removeClass('mgDateTimePicker-dom-btn-empty-before-filled');
				            }
				          ...
				          else
				          {
				        	i = cmpLo.getDate();
				            td.text(i);
				            td.removeClass('mgDateTimePicker-dom-btn-empty mgDateTimePicker-dom-btn-empty-above-filled mgDateTimePicker-dom-btn-empty-before-filled '+
				            				'mgDateTimePicker-dom-btn-empty-after-filled mgDateTimePicker-dom-btn-empty-below-filled ' +
				            				'ui-state-default ui-state-disabled');
				            td.addClass('mgDateTimePicker-dom-btn-filled ui-state-default');
				            td.mgDateTimePicker_current( i == current,
					        		((!_this.earliest)||(cmpHi.getTime()>=_this.earliest)) &&
					        		((!_this.latest)||(cmpLo.getTime()<=_this.latest)) );
				            cmpLo.setDate(i+1);
				            cmpHi.setDate(i+1);
        ...
        var hr = this.time.getHours();
		    $('#'+this.id+' .mgDateTimePicker-hr-btn').each(
		      function()
		      {
                ...
                cmpLo.setHours(i);
                cmpHi.setHours(i);
            $(this).mgDateTimePicker_current( hr == i,
		        	((!_this.earliest)||(cmpHi.getTime()>=_this.earliest)) &&
		        		((!_this.latest)||(cmpLo.getTime()<=_this.latest)) );
            if ( i < 23 )
        ...
        cmpHi.setHours( this.time.getHours() );
		    ...
		    $('#'+this.id+' .mgDateTimePicker-min-ten-btn:not(.mgDateTimePicker-min-ten-btn-empty)').each(
		      function()
		      {
		        $(this).mgDateTimePicker_current( this.innerHTML == tens,
		        		((!_this.earliest)||(cmpHi.getTime()>=_this.earliest)) &&
		        		((!_this.latest)||(cmpLo.getTime()<=_this.latest)) );
		    ...
		    $('#'+this.id+' .mgDateTimePicker-min-one-btn:not(.mgDateTimePicker-min-one-btn-empty)').each(
		      function()
		      {
		        $(this).mgDateTimePicker_current( this.innerHTML == ones,
		        		((!_this.earliest)||(cmpHi.getTime()>=_this.earliest)) &&
		        		((!_this.latest)||(cmpLo.getTime()<=_this.latest)) );
		    ...
		    $('#'+this.id+' .mgDateTimePicker-sec-ten-btn:not(.mgDateTimePicker-sec-ten-btn-empty)').each(
		      function()
		      {
		        $(this).mgDateTimePicker_current( this.innerHTML == tens,
		        		((!_this.earliest)||(cmpHi.getTime()>=_this.earliest)) &&
		    ...
		    $('#'+this.id+' .mgDateTimePicker-sec-one-btn:not(.mgDateTimePicker-sec-one-btn-empty)').each(
		      function()
		      {
		        $(this).mgDateTimePicker_current( this.innerHTML == ones,
		        		((!_this.earliest)||(cmpHi.getTime()>=_this.earliest)) &&
		    ...
		    if ( this.dY )
		    {
				...
				totH = this.yLab.mgDateTimePicker_height(true) + this.dY.mgDateTimePicker_height(true);
		    }
		    if ( this.dMo )
		    {
		    	...
		    	if ( dMoW > totW )
		    		totW = dMoW;
		    	totH += this.hMo.mgDateTimePicker_height(true) + this.dMo.mgDateTimePicker_height(true);
		    }
		    if ( this.dDoM )
		    {
			    ...
			    totH += this.hDoM.mgDateTimePicker_height(true) + this.dDoM.mgDateTimePicker_height(true);
		    }
		    if ( this.dD )
		    {
		    ...
		    if ( this.dH )
		    {
		    	...
		    	timeW += w + 1;
		    	h = this.dH.mgDateTimePicker_height(true);
		    	if ( h > timeH )
		    		timeH = h;
		    }
		    if ( this.dM )
		    {
		        ...
		        timeW += w + 1;
		        h = this.dM.mgDateTimePicker_height(true);
		        if ( h > timeH )
		        	timeH = h;
		    }
		    if ( this.dS )
		    {
		        ...
		        timeW += w + 1;
		        h = this.dS.mgDateTimePicker_height(true);
		        if ( h > timeH )
		        	timeH = h;
		    }
		    if ( this.dO )
		    {
		        ...
		        if ( timeW < w+1 )
		        	timeW = w+1;
		        timeH += this.dO.mgDateTimePicker_height(true);
		    }
		    if ( this.dT )
			{
		    ...
		    totH += this.bMinH;
		    totW += this.bMinW;
		    totH += this.hTitle.mgDateTimePicker_height(true) + this.wMinH;
		    totW += this.wMinW;
		...
		updODiv: function(fBtn)
		{
		    ...
		    this.oDiv.find('.mgDateTimePicker-off-off-btn').each(
		      function()
		      {
		    	  if ( this.mgDateTimePicker_offMin == _this.offMin )
		    	  {
		    		  if ( this.mgDateTimePicker_offSI == _this.offSI )
		    			  $(this).mgDateTimePicker_current(matched=true,true);
		    		  else
		    	      {
		    			  $(this).mgDateTimePicker_current(false,true);
		    			  if ( def == null )
			    			  def = $(this);
		    	      }
		    	  }
		    ...
		    if ( ( ! matched ) && ( def != null ) )
		    	def.mgDateTimePicker_current(true,true);
		...
		updYDiv: function(fBtn)
		{
		    ...
		    yearValue = mgDateTimePicker.pad( yearValue, 4 );
		    var eY = _this.earliest && new Date(_this.earliest).getFullYear();
		    var lY = _this.latest && new Date(_this.latest).getFullYear();
	    
		    i = 0;
		    this.yDiv.find('.mgDateTimePicker-mil-btn').each(
		      function()
		      {
		    	legal = ( ((!_this.earliest)||(era*(i+(era<0?0:999))>=eY)) && ((!_this.latest)||(era*(i+(era>0?0:999))<=lY)) );
		        $(this).mgDateTimePicker_current( this.innerHTML == yearValue.substring(0,1), legal );
		        i += 1000;
		      } );

		    i = (Math.floor(yearValue/1000)*1000);
		    this.yDiv.find('.mgDateTimePicker-cent-btn').each(
		      function()
		      {
			    	legal = ( ((!_this.earliest)||(era*(i+(era<0?0:99))>=eY)) && ((!_this.latest)||(era*(i+(era>0?0:99))<=lY)) );
		        $(this).mgDateTimePicker_current( this.innerHTML == yearValue.substring(1,2), legal );
		        i += 100;
		      } );

		    i = (Math.floor(yearValue/100)*100);
		    this.yDiv.find('.mgDateTimePicker-dec-btn').each(
		      function()
		      {
			    	legal = ( ((!_this.earliest)||(era*(i+(era<0?0:9))>=eY)) && ((!_this.latest)||(era*(i+(era>0?0:9))<=lY)) );
		        $(this).mgDateTimePicker_current( this.innerHTML == yearValue.substring(2,3), legal );
		        i += 10;
		      } );

		    i = (Math.floor(yearValue/10)*10);
		    this.yDiv.find('.mgDateTimePicker-yr-btn').each(
		      function()
		      {
			    legal = ( ((!_this.earliest)||(era*i>=eY)) && ((!_this.latest)||(era*i<=lY)) );
		        $(this).mgDateTimePicker_current( this.innerHTML == yearValue.substring(3), legal );
		        i += 1;
	          } );
		    
		    this.yDiv.find('.mgDateTimePicker-bce-btn').each(
		      function()
		      {
		    	  $(this).mgDateTimePicker_current( era < 0, (!_this.earliest) || ( _this.earliest < 0 ) );
		      } );
		    this.yDiv.find('.mgDateTimePicker-ce-btn').each(
		      function()
		      {
		    	$(this).mgDateTimePicker_current( era > 0, (!_this.latest) || ( _this.latest > 0 ) );
		      } );
